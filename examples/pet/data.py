...
def create_dataloader(
    ...
    sampler_class = DistributedBatchSampler if mode == 'train' else BatchSampler
    batch_sampler = sampler_class(dataset, batch_size, shuffle= mode=='train')
...
def convert_example(example, tokenizer, max_seq_length=512, is_test=False):
    ...
    if "<unk>" in sentence1:
        ...
        # Step2: Insert "[MASK]" to input_ids based on start_mask_position
        input_ids = input_ids[0:start_mask_position] + mask_ids + input_ids[
            start_mask_position:]
        ...
        encoded_inputs = tokenizer(text=sentence1, max_seq_len=max_seq_length)
        sentence1_input_ids = encoded_inputs["input_ids"][1:]
        input_ids = sentence1_input_ids + input_ids
    ...
    assert len(input_ids) == len(
        token_type_ids), "length input_ids, token_type_ids must be equal"
    ...
    if is_test:
        return input_ids, token_type_ids, mask_positions
    else:
        ...
        return input_ids, token_type_ids, mask_positions, mask_lm_labels
...
def convert_chid_example(example, tokenizer, max_seq_length=512, is_test=False):
    ...
    if is_test:
        return input_ids, token_type_ids, mask_positions, candidate_labels_ids
    else:
        mask_lm_labels = tokenizer(
            text=text_label, max_seq_len=max_seq_length)["input_ids"][1:-1]
        ...
        return input_ids, token_type_ids, mask_positions, mask_lm_labels, candidate_labels_ids
